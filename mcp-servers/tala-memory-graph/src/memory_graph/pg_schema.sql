-- pg_schema.sql: Graph tables for the tala-memory-graph PostgreSQL backend.
--
-- Table names are prefixed with graph_ to coexist with the canonical
-- entity/relationship tables created by the main Electron app migrations.
-- This file is executed by PostgresGraphStore._init_db() on first connect
-- and is also registered as electron/migrations/006_graph_nodes_edges.sql
-- so the canonical MigrationRunner can apply it.

...
| **PolicyAuthoringPanel** | **AppSettings.guardrailPolicy** | Settings IPC (`api.saveSettings`) | Writes structured `GuardrailPolicyConfig` (profiles, rules, scopes, validator bindings) to `app_settings.json` via `tala.saveSettings()`. Authoring surface only — no enforcement runs in the renderer. Config is versioned (`version: 1`). PolicyGate is the future runtime consumer. Source: `shared/guardrails/guardrailPolicyTypes.ts`. | Medium (Config) |
| **WorkflowRegistry** | **ToolService** | Internal API | `executeWorkflow(id, initialArgs?, executionMode?)` — executes registered multi-step workflows via `ToolService.getToolDefinition().execute()`. `executionMode` defaults to `'system'`. `policyGate.assertSideEffect({ actionKind: 'workflow_action', ... })` fires before each step; `PolicyDeniedError` propagates to caller. | Critical (Control) |

...
describe('SessionPersistenceSelfKnowledge', () => {
    beforeEach(() => {
    ...
    it('persists self-knowledge assistant response via session publication seam', async () => {
        const agent = {
        ...
        vi.spyOn(kernel as any, 'classifyExecution').mockImplementation((_request: any, meta: any) => {
            meta.turnArbitration = {
                turnId: 'turn-self-knowledge-session',
                mode: 'conversational',
                source: 'test',
                confidence: 1,
                reasonCodes: ['test:session_persist'],
        ...
        expect(agent.publishAuthorityTurnToSession).toHaveBeenCalledWith(

...
export class AutonomousRunOrchestrator {
    ...
    private async _executeGoalPipeline(
        ...
        try {
            ...
            const govDecision = await this._runGuardedAutonomyOperation({
                runId: run.runId,
                subsystemId: goal.subsystemId,
                operationName: 'governance_polling',
                maxAttempts: 1,
                shouldCountFailureForCircuit: () => false,
                execute: async () =>
                    this.governanceAppService.evaluateForProposal(
            ...
            const execResponse = await this._runGuardedAutonomyOperation({
                runId: run.runId,
                subsystemId: goal.subsystemId,
                operationName: 'execution_start',
                maxAttempts: 1,
                execute: async () => this.executionOrchestrator.start({
                    proposalId: proposal.proposalId,
            ...
            const terminalStatus = await this._runGuardedAutonomyOperation({
                runId: run.runId,
                subsystemId: goal.subsystemId,
                operationName: 'execution_wait',
                maxAttempts: 1,
                execute: async () => this._waitForExecution(execResponse.executionId),
            });
            await enforceSideEffectWithGuardrails(
                'autonomy',
                {
    ...
    private async _runGuardedAutonomyOperation<T>(
        opts: {
            runId?: string;
            subsystemId: string;
            ...
            maxAttempts?: number;
            execute: () => Promise<T>;
            shouldRetry?: (error: unknown, failureKind: GuardrailFailureKind, attempt: number) => boolean;
            shouldCountFailureForCircuit?: (error: unknown, failureKind: GuardrailFailureKind) => boolean;
        },
    ): Promise<T> {
        const breaker = AutonomousRunOrchestrator.guardrailBreakerStore.get(
            `autonomy:${opts.subsystemId}:${opts.operationName}`,
            {
                failureThreshold: 3,
                resetAfterMs: 20_000,
            },
        );
        const guarded = await executeWithRuntimeGuardrails<T>({
            domain: 'autonomy',
            operationName: opts.operationName,
            targetKey: `${opts.subsystemId}:${opts.operationName}`,
            executionId: opts.runId,
            maxAttempts: Math.max(1, opts.maxAttempts ?? 1),
            circuitBreaker: breaker,
            classifyFailure: (error): GuardrailFailureKind => {
                if (error instanceof PolicyDeniedError) return 'policy_denied';
                return 'runtime_error';
            },
            shouldRetry: (error, attempt, failureKind) =>
                opts.shouldRetry?.(error, failureKind, attempt) ?? false,
            shouldCountFailureForCircuit: (error, failureKind) =>
                opts.shouldCountFailureForCircuit?.(error, failureKind) ??
                (failureKind !== 'policy_denied' && failureKind !== 'governance_blocked'),
            execute: async () => opts.execute(),
        });
        if (!guarded.ok) {
            throw (guarded.error ?? new Error(`Guarded autonomy operation failed: ${opts.operationName}`));
        }
